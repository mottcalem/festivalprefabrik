...
export default function SiteHeader({locale='tr'}){
  ...
  return <><div className="topline"><span>{en?'Niksar / Tokat':'Niksar / Tokat'}</span><span>{en?'Production and installation across Türkiye':"Türkiye'nin her yerine üretim ve montaj"}</span><a href={`tel:${data.phone.replaceAll(' ','')}`}><PhoneIcon/> {data.phone}</a><a href={`tel:${data.mobile.replaceAll(' ','')}`}>{data.mobile}</a></div><header className="inner-header">
    ...
    <nav className={open?'open':''}>
      <div className="nav-group"><a href={en?'/en/about':'/festival-prefabrik/kurumsal'}>{en?'About Us':'Hakkımızda'} <ChevronDownIcon/></a><div className="dropdown"><a href={en?'/en/about':'/festival-prefabrik/kurumsal'}>{en?'Corporate':'Kurumsal'}</a><a href={en?'/en/history':'/festival-prefabrik/tarihce'}>{en?'History':'Tarihçe'}</a></div></div>
      <div className="nav-group"><a href={en?'/en/models':'/modeller?kategori=prefabrik-tek-kat'}>{en?'Prefabricated House':'Prefabrik Ev'} <ChevronDownIcon/></a><div className="dropdown"><a href={en?'/en/models':'/modeller?kategori=prefabrik-tek-kat'}>{en?'Single Storey':'Tek Kat'}</a><a href={en?'/en/models':'/modeller?kategori=prefabrik-cift-kat'}>{en?'Two Storey':'Çift Kat'}</a><a href={en?'/en/models':'/modeller?kategori=konteyner'}>{en?'Container':'Konteyner'}</a></div></div>
      <div className="nav-group"><a href={en?'/en/models':'/modeller?kategori=celik-tek-kat'}>{en?'Steel House':'Çelik Ev'} <ChevronDownIcon/></a><div className="dropdown"><a href={en?'/en/models':'/modeller?kategori=celik-tek-kat'}>{en?'Single Storey':'Tek Kat'}</a><a href={en?'/en/models':'/modeller?kategori=celik-cift-kat'}>{en?'Two Storey':'Çift Kat'}</a></div></div>
