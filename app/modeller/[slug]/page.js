...
export default async function ModelDetail({params}){const {slug}=await params;const p=data.projects.find(x=>x.slug===slug);if(!p)notFound();const others=data.projects.filter(x=>x.slug!==slug&&x.category===p.category).slice(0,3);return <main><SiteHeader/>
  ...
  <section className="detail-visual wrap"><ModelGallery key={p.slug} images={[...new Set([...(p.images?.length ? p.images : [p.image]), p.plan].filter(Boolean))]} title={p.title} locale="tr"/><ModelVideo src={p.video} youtubeId={modelVideos[p.code]} locale="tr"/><aside className="model-summary"><span>Model özeti</span>{[['Model alanı',p.area],['Oda planı',p.rooms],['Banyo',p.bathrooms],['Kat',p.floors],['Tahmini teslim',p.delivery]].filter(([,v])=>v).map(([k,v])=><div className={v === p.area ? "summary-area" : "summary-row"} key={k}><small>{k}</small><b>{v}</b></div>)}<a href={`/teklif?model=${encodeURIComponent(p.title)}`}>Bu model için teklif al <ArrowRightIcon/></a></aside></section>
